...
(function () {
  ...
  const previewImage = document.querySelector('#designPreviewImage');
  const previewMedia = document.querySelector('.design-work-preview-media');
  /*
      이미지 밑에 있던
      PUBLISHING
      Web Publishing
      ...
      부분은 사용하지 않음
  */
  const previewInfo = document.querySelector('.design-work-preview-info');
  if (previewInfo) {previewInfo.hidden = true;}
  /* =====================================================
     기존 가운데 글씨 숨김
  ====================================================== */
  const orbitCenter = document.querySelector('#designOrbitCenter');
  if (orbitCenter) {orbitCenter.hidden = true;}
    ...
    */
    ...
    orbitItems.forEach((item, index) => {
      ...
      /*
          ...
          angle + 90 사용
      */
      item.style.setProperty('--item-rotation','0deg');
    }
    );
  }
  /* 최초 위치 */
  ...
  function updatePreviewRatio() {
    ...
    if (image?.naturalWidth && image.naturalHeight) {
      previewMedia.style.setProperty('--preview-ratio', image.naturalWidth / image.naturalHeight);
    } else {
      previewMedia.style.removeProperty('--preview-ratio');
    }
  }
  ...
  function moveSlide(direction) {slideIndex = (slideIndex + direction + slideCount) % slideCount;
    updateControls();
    restartAutoplay();
  }
  ...
  function selectItem(
    ...
    orbitItems.forEach(
      (button) => {const active = button === item;
  ...
  orbitItems.forEach(
    (item) => {item.setAttribute('type','button' );
